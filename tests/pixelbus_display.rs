...
/// Slice 100: a client area wide enough for the grid but too short for its final
...
/// the whole check exists to name and is worse for being partial: the final
/// native-binding blocks are drawn past the client edge and fail closed, while
/// the blocks above them keep decoding perfectly.
#[test]
fn a_grid_too_short_for_its_final_row_exceeds() {
    ...
    assert_eq!(
        grid_h,
        DEFAULT_BLOCK_PX * 3,
        "the S100 grid should be three rows tall"
    );

    // Wide enough for a full row, one pixel short of all three rows.
    let surface = Size::new(DEFAULT_BLOCK_PX * COLUMNS, grid_h - 1);
    assert_eq!(
        grid_fit(grid, surface),
        GridFit::Exceeds { grid, surface },
        "a client area too short for the final row must be reported"
    );

    // Exactly three rows tall fits, because equality on either axis fits.
