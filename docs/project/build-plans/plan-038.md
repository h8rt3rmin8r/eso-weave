...
7. S074 is in progress and implements the user-initiated catalog update,
   collector handshake, immutable user-data selection, rollback, recovery, and
   redacted receipt workflow tracked by issue #118.
8. Later slices continue the encounter pipeline through issues #132 through
   #136 according to
...
Issue #132 is also eligible, but issue #118 remains next in the order established
by epic #111 and turns reviewed candidates into an explicit safe user workflow. Installed v0.15.1 verification in issue #110, catalog field
