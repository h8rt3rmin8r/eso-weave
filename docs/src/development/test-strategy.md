...
| Local extension contract | `tests/local_extension_contract.rs`, `tests/local_service.rs`, `tests/player_state.rs`, `tests/database_query.rs` | Published field inventory completeness, safe examples, authenticated HTTP and official RMCP parity, bounded queries, lifecycle recovery, disconnects, and shutdown |
