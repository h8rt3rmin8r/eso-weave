...
- S093 adds a first-class ESO Weave Data lifecycle row directly beneath
  ...
  controls. A separate evidence row keeps ownership, compatibility, configured
  enablement, loading, reload need, ESO runtime, catalog activity, and encounter
  activity explicit without manufacturing a live addon channel (issue #185).
...
- 2026-09-03: Remove `GetItemLinkOnUseAbilityInfo` as a quickslot acceptance
  ...
  item-link ID. Reject a B20 sample when a widened user tolerance matches multiple
  state codes. One dedicated B20 discriminant is the smallest protocol change that
  keeps classification independent from cooldown and identity.
