...
- **Principle II, Safety-Critical Surfaces**: PASS. The design does not modify reader routing, authoritative state, controller evidence, or input gates. Presentation types have no route back to action code.
...
`view_at(now_ms)` is the deterministic projection seam; ordinary `view()` delegates with the existing injected-clock `now_ms()`. When evidence is coherent, the model records the new presentation and clears stale state. On a covered loss, it starts one interval from that loss if a snapshot exists and retention is positive. A later cause changes only the explanation. Expiry is always recomputed from the original loss timestamp and current configured interval, so settings can shorten but never restart or prolong time already elapsed.
...
Reader and process loops continue to close gates and clear authoritative observations exactly as they do today. No presentation cache is passed into `WeaveEngine`, Fishing, Auto Potion, `InputEngine`, routing, persistence, or logs.
