...
- [x] T045 Wait for CI, CodeQL, dependency, Codex, and security results; address every comment and resolve every thread.
- [x] T046 Trigger exactly one authorized second `@Codex review` round and address its results.
- [x] T047 Confirm all required checks green, zero unresolved threads, and ask the operator for final review and merge.
