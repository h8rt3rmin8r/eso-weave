...
Generation must complete with 28 PNGs and `capture-manifest.json`. S084 selects only the seven `--dark--wide.png` files named in the screenshot plan and losslessly removes empty lower canvas through `scripts/curate-documentation-captures.ps1`.

## Publication

- Application and supplied PNGs live in `docs/src/assets/screenshots/`.
...
- an invalid PNG signature or IHDR;
