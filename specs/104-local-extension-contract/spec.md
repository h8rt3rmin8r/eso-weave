...
- **FR-012**: Disable and application exit MUST cancel acceptance and in-flight adapter work, stop new queries, remove or invalidate discovery, release listener and database handles, and join the owner thread within 3 seconds. Timeout MUST remain visible as a failure.
- **FR-013**: The contract MUST define one canonical player-state schema `1.0.0` with stable field paths, types, availability rules, source metadata, protocol metadata, observation time, freshness state, snapshot revision, and schema version.
...
- **FR-023**: Query results MUST preserve SQLite null, integer, real, text, and blob types. Blobs MUST use base64 plus an explicit type marker. Column order and duplicate names MUST remain representable.
